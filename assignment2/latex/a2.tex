...
\begin{shaded}
\begin{answer} The derivative of Leaky Rectified Linear Unit (Leaky ReLU) activation function:
\[ 
    \frac{{d\max(\alpha x, x)}}{dx} =
    \begin{cases}
        \alpha \text{ if } x < 0 \\
        1 \text{ if } x > 0
    \end{cases}
...
\begin{shaded}
\begin{answer} The derivative of $\sigma(x)$ with respect to $x$, where $x$ is a scalar:
\begin{align}
    \frac{d \sigma(x)}{dx} &= \frac{d}{dx}\left(\frac{e^x}{e^x + 1}\right) \\
    &= \frac{e^x (e^x + 1) - e^x e^x}{(e^x + 1)^2} \\
    &= \frac{e^x}{(e^x + 1)^2} \\
    &= \frac{(e^x + 1) - e^x}{e^x + 1} \times \frac{e^x}{e^x + 1} \\
    &= \left(1 - \frac{e^x}{e^x + 1}\right) \times \frac{e^x}{e^x + 1} \\
    &= (1 - \sigma(x))\sigma(x)
\end{align}
\end{answer}
\end{shaded}
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item We could reuse the calculation of sigmoid and the inner product:
    \begin{equation}
        \frac{\partial \bm J_{\text{neg-sample}(\bm v_c, o, \bm U)}}{\partial \bm v_c} = -\bm{U}_{o, \{w_1, \dots, w_K\}} \left( \bm 1 - \sigma(\bm{U}_{o, \{w_1, \dots, w_K\}}^\top \bm v_c) \right)
    \end{equation}
    \begin{equation}
        \frac{\partial \bm J_{\text{neg-sample}(\bm v_c, o, \bm U)}}{\partial \bm U} = -\frac{\bm{U}_{o, \{w_1, \dots, w_K\}}}{|\bm{U}_{o, \{w_1, \dots, w_K\}}|} \odot \bm v_c \left( \bm 1 - \sigma\left(\bm{U}_{o, \{w_1, \dots, w_K\}}^\top \bm v_c\right) \right)^\top
    \end{equation}

    \item \textbf{Negative Sampling} is much more efficient because it samples a fixed number $K$ of vocabulary words that are involved in loss computation along with the true outside word, whereas \textbf{Softmax} involves normalizing the unnormalized probability meaning it has to go through all word vectors in the vocabulary (which can have $10^6$ - $10^7$ entries).
\end{enumerate}
\end{answer}
\end{shaded}
...
\begin{shaded}
\begin{answer} The partial derivative of the $\bm J_{\text{neg-sample}}(\bm v_c, o, \bm U)$ without the assumption that the $K$ sampled words are distinct:
\begin{align}
    \frac{\partial \bm J_{\text{neg-sample}}(\bm v_c, o, \bm U)}{\partial \bm u_{w_s}} = \sum_{\substack{w\in\text{Vocab} \\ w=w_s}}^K \left( 1 - \sigma(-\bm u_{w}^\top \bm v_c) \right) \bm v_c
\end{align}
\end{answer}
\end{shaded}
