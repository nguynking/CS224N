...
\end{lstlisting}
    ...
    \ifans{The masks will assign $-\infty$ values to the attention scores at positions corresponding to 'pad' tokens in the input. This will give the probability of the 'pad' tokens to be 0 after applying softmax, which means the 'pad' token embeddings will not affect the attention outputs. It is necessary because the 'pad' tokens are just additional elements we use to make the length of sentences equal and they do not appear in actual sentences. Thus we don't want these tokens to affect the output probability distribution. \newline
    
    \textbf{Another solution:} 
    \begin{enumerate}
        \item \textbf{What effect:} for every batch, those attention scores which have corresponding zero-padded embeddings are set to $-\infty$. This way, during the calculation of \textit{attention distributions} $\alpha_t$, the probabilities are calculated with corresponding non-padded words whereas the scores with corresponding padded words are negligible. Finally, during the calculation of \textit{attention outputs} $A_t$, for every batch only the addition of those hidden states matter which are not multiplied by a probability close to 0.
        \item \textbf{Why necessary:} using masks in this way is an efficient way to determine the true \textit{attention distribution} that only involves the non-padded entries. Involving padded entries would result in false \textit{attention} representation.
    \end{enumerate}}
\end{parts}
...
\end{lstlisting}
    ...
    \ifans{Corpus BLEU: 20.349133759154903 (with early stopping after 4 epochs, 18800 iterations)}
    ...
    \ifans{
    \begin{subparts}
        \subpart An advantage of \textit{dot product} attention compared to \textit{multiplicative attention} is that it does not have any learnable parameters and it is a vector dot product, meaning it is fast to compute. A disadvantage, however, is that a simple dot product is not sufficient to capture what parts of $\textbf{s}_t$ and what part of $\textbf{h}_t$ to pay attention to because it is a simple piece-wise similarity.
        \subpart An advantage of \textit{additive attention} compared to \textit{multiplicative attention} is that similarity is computed in a non-linearly transformed space where both $\textbf{h}_i$ and $\textbf{s}_t$ have their own learnable weights. This add more flexibility in terms of parameter space. A disadvantage is that the computation is very expensive.
    \end{subparts}
    }
\end{parts}
