...
class DownProjectBlock(nn.Module):
    ...
    def forward(self, x_input):
        ...
        # Apply cross-attention to x_input and self.C
        x = self.C + self.attn(x_kv=self.ln1(x_input), x_q=self.ln1(self.C))
        x = x + self.mlp(self.ln2(x))
...
class UpProjectBlock(nn.Module):
    ...
    def forward(self, y, x_input):
        ...
        x = x_input + self.attn(x_kv=self.ln1(y), x_q=self.ln1(x_input))
        x = x + self.mlp(self.ln2(x))
