...
\begin{parts}
    ...
    \ifans{A sentence containing $n$ words will be parsed in $2n$ steps because the dependency parser is linear in time. In other words, it works with pairs of words and it does not support nested dependencies, thus it will take $n$ steps for all the \texttt{SHIFT} operations and another $n$ steps for \texttt{LEFT-ARC} and \texttt{RIGHT-ARC} operations.} \newline
...
\begin{itemize}
    \item \textbf{Error type}: Modifier Phrase Attachment Error 
    \item \textbf{Incorrect dependency}: left $\rightarrow$ early 
    \item \textbf{Correct dependency}: Friday $\rightarrow$ early
\end{itemize}
...
\ifans{Part-of-speech (POS) tags are useful features in parsers because they provide information about the grammatical function of words in a sentence. This information can be use to disambiguate between possible parses, and to make more accurate predictions about the structure of a sentence.}
